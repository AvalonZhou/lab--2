...
<%
    for(Student student : list)
    {
       ...
       %>
    <tr>
        <td><%=sid%></td><td><%=name%></td><td><%=gender.equals("m")?"男":"女"%>
            </td><td><%=age%></td><td><%=birthday%></td>
        <td><a href="modify.jsp?sid=<%=sid%>">修改</a></td><td><a onclick="return
            confirm('确定要删除<%=sid%>号同学吗？')" href="student-servlet?action=remove&sid=<%=sid%>">删除</a></td>
    </tr>
<%
    }
%>
